#!/bin/bash
# 此文件用 LMDeploy 加速推理来测评 Qwen3-VL-8B-Instruct 模型
# 聚焦主题：图像【细粒度推理】+【视觉问答 VQA】相关基准
# 与 eval_qwen3vl_thinking.sh 使用同一套基准与指标，便于横向对比。
# 在 vlmeval 虚拟环境中启动
#
# 启动方式参考官方文档：
#   https://github.com/open-compass/VLMEvalKit/blob/main/docs/zh-CN/EvalByLMDeploy.md
#
# 与 eval_qwen3vl_thinking.sh 的区别：
#   - 原脚本用 torchrun 本地加载权重（use_vllm）推理。
#   - 本脚本先用 `lmdeploy serve api_server` 起 OpenAI 兼容服务，
#     再让 run.py 通过 `--base-url` 走 LMDeployAPI 调用（无需改 config.py）。
#
...
# ============ 可配置参数 ============
MODEL_NAME=${MODEL_NAME:-"Qwen3-VL-8B-Instruct"}          # run.py 的 --model，需与下面 --model-name 一致
MODEL_PATH=${MODEL_PATH:-"/mnt/data/VLM/Qwen3-VL-8B-Instruct"}    # 本地权重路径
SERVER_PORT=${SERVER_PORT:-23334}                         # LMDeploy 服务端口
TP=${TP:-4}                                               # 张量并行 / 使用的 GPU 数
API_NPROC=${API_NPROC:-64}                                # run.py 并发请求数
GPUS=${GPUS:-"0,1,2,3"}                                       # 可见 GPU
START_TIMEOUT=${START_TIMEOUT:-600}                       # 等待服务就绪的最长秒数
# Qwen3-VL 生成参数（对齐 config.py 中的设置）
...
python run.py \
  ...
  --extra-body '{"presence_penalty": 1.5}' \
  --verbose \
